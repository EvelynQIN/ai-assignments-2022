...
from sklearn.gaussian_process.kernels import Matern, RationalQuadratic, WhiteKernel, RBF
import numpy as np
...
class Model(object):
    ...
    def __init__(self):
        ...
        self.kernel = Matern(nu=0.5, length_scale=2.0) + RationalQuadratic(length_scale=0.09, alpha=1.0) + WhiteKernel(noise_level=0.01)  # 14.489
    ...
    def make_predictions(self, test_features: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        norm_test_feature = self.scaler_feature.transform(test_features)
        k_test_train = self.kernel(norm_test_feature, self.norm_train_feature)
        test_y_mean = k_test_train @ self.k_inv_label
        test_y_cov = self.kernel(norm_test_feature) - k_test_train @ np.linalg.solve(self.k_train, k_test_train.T)
        # sample = np.random.multivariate_normal(0 * test_y_mean.reshape(-1), test_y_cov)
        # sample = sample.reshape(-1) * self.scaler_label.scale_
        gp_mean = self.scaler_label.inverse_transform(test_y_mean).reshape(-1)
        gp_std = np.sqrt(np.diagonal(test_y_cov)) * self.scaler_label.scale_
        sample = np.random.randn(test_features.shape[0]) * gp_std

        #TODO: Use the GP posterior to form your predictions here
        predictions = gp_mean + np.abs(sample) * 0.2
    ...
    def fitting_model(self, train_GT: np.ndarray, train_features: np.ndarray):
        ...
        #anomaly deletion: 17 records with y < 0
        valid_ids = train_GT > 0
        train_features = train_features[valid_ids]
        train_GT = train_GT[valid_ids]

        self.norm_train_feature = self.scaler_feature.fit_transform(train_features)
        norm_train_label = self.scaler_label.fit_transform(train_GT.reshape(-1, 1))
        self.k_train = self.kernel(self.norm_train_feature)
        self.k_inv_label = np.linalg.solve(self.k_train, norm_train_label)
        # pass
...
def perform_extended_evaluation(model: Model, output_dir: str = '/results'):
    ...
    ax_gp.plot_surface(X=grid_lon, Y=grid_lat, Z=gp_mean, facecolors=cm.get_cmap()(gp_stddev / vmax_stddev), rcount=EVALUATION_GRID_POINTS_3D, ccount=EVALUATION_GRID_POINTS_3D, linewidth=0, antialiased=False)
