...
def mlp(sizes: List[int], activation: nn.Module, output_activation: nn.Module = nn.Identity):
    """
    ...
    for i in range(len(sizes) - 2):
        mod_list += [nn.Linear(in_features=sizes[i], out_features=sizes[i + 1]), activation()]
    mod_list += [nn.Linear(in_features=sizes[-2], out_features=sizes[-1]), output_activation()]
    # print(mod_list)
...
class Actor(nn.Module):
    ...
    def _distribution(self, obs: torch.Tensor):
        ...
        # raise NotImplementedError
        # print(obs, obs.size())
        return distributions.categorical.Categorical(logits=self.logits_net(obs))

    def _log_prob_from_distribution(self, pi: distributions.Distribution, act: torch.Tensor):
        """
    ...
    def forward(self, obs, act=None):
        ...
        if act is not None:
            log_prob = self._log_prob_from_distribution(pi=pi, act=act)
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        self.tdres_buf[path_slice] = discount_cumsum(x=delta, discount=self.gamma * self.lam)

        # TODO: Implement discounted rewards-to-go calculation.
        # Hint: use the discount_cumsum function
        self.ret_buf[path_slice] = discount_cumsum(x=self.rew_buf[path_slice], discount=self.gamma)
...
class Agent:
    ...
    def __init__(
        ...
        self.l = 4  # layer number of networks
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            ...
            act = pi.sample()
            v = self.critic(state)
            logp = self.actor._log_prob_from_distribution(pi=pi, act=act)
    ...
    def get_action(self, obs):
        ...
        pi = self.actor._distribution(torch.tensor(obs, dtype=torch.float32))
        act = pi.sample()

        return act


def train(env, seed=114514):
    """
    Main training loop.
    ...
    for epoch in range(epochs):
        ...
        # parameters is the policy gradient. Then call loss.backward() and actor_optimizer.step()
        ...
        for _ in range(3):
            pi = agent.actor._distribution(obs)
            logp = agent.actor._log_prob_from_distribution(pi, act)

            # ratio = 1.0
            ratio = (logp - lopg_old).exp()
            cliped_ratio = torch.min(torch.max(ratio, torch.tensor(1 - eps)), torch.tensor(1 + eps))

            # print(ratio, cliped_ratio)

            A = torch.min(ratio * data["tdres"], cliped_ratio * data["tdres"])

            neg_J = -(A.detach() * logp).sum() - A.sum()
            neg_J.backward()
