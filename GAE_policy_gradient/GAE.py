...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    for l in range(len(sizes) - 1):
        ...
        if l == len(sizes) - 2: # if the output layer
...
class Actor(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        super().__init__()
        self.logits_net = mlp([obs_dim] + list(hidden_sizes) + [act_dim], activation) # return the logits over the action space
...
class Agent:
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            ...
            act = pi.sample() # sample one action based on the distri.
            logp = self.actor(state, act)[1]
            v = self.critic(state)
...
def train(env, seed=0):
    ...
    # # the regulate the strength of the constrain for updating the critic network
    # epsilon = 1
    ...
    for epoch in range(epochs):
        ...
        loss_policy = - (logp_new @ data['tdres']) / num_ep
        ...
        for _ in range(100):
            ...
            v_new = agent.critic(data['obs'])
            # v_old = data['v']
            # delta2 = torch.mean((v_old - data['ret']) ** 2) 
            # constraint = torch.mean(((v_new - v_old) ** 2) / (2 * delta2))
            #print(constraint)
            # loss_v = torch.sum((v_new - data['ret']) ** 2) + epsilon * constraint
            loss_v = torch.sum((v_new - data['ret']) ** 2) 
            loss_v.backward()
